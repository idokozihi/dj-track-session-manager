...
bool LRUCache::put(PointerWrapper<AudioTrack> track) {
    ...
    for(size_t i=0 ; i<max_size ; i++){
        if(slots[i].isOccupied()){
            ...
            if(existing->get_title() == track->get_title()) {
                access_counter++;
    ...
    if(empty_index==max_size){
        size_t lru_index = findLRUSlot();
        slots[lru_index].clear();
        slots[lru_index].store(std::move(track), ++access_counter);
        return true;
    }
    access_counter++;
...
size_t LRUCache::findLRUSlot() const {
    ...
    unsigned long long min_access = (unsigned long long)-1;
    for (size_t i = 0; i < max_size; i++){
