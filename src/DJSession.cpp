...
{
    AudioTrack *load_track = library_service.findTrack(track_name);
    if (load_track == nullptr)
    {
    ...
    int result = controller_service.loadTrackToCache(*load_track);
    if (result == 1)
    {
        stats.cache_hits++;
    }
...
bool DJSession::load_track_to_mixer_deck(const std::string &track_title)
{
    ...
    std::cout << std::endl;
    AudioTrack *load_track = controller_service.getTrackFromCache(track_title);
    if (load_track == nullptr)
    {
    ...
    int result = mixing_service.loadTrackToDeck(*load_track);
    if (result == 1)
    {
        stats.deck_loads_b++;
        stats.transitions++;
    ...
    {
        std::cout << "[ERROR] Deck load failed for track: \""
                  << track_title << "\"\n";
        stats.errors++;
        return false;
    }
...
void DJSession::simulate_dj_performance()
{
    ...
    if (play_all == true)
    {
        ...
        for (const auto &track : session_config.playlists)
        {
            playlist_names.push_back(track.first);
        }
        std::sort(playlist_names.begin(), playlist_names.end());

        for (const auto &playlist_name : playlist_names)
        {
            bool a = load_playlist(playlist_name);
            if (!a)
            {
                std::cout << "error";
                stats.errors++;
            ...
            for (const auto &track_title : track_titles)
            {
                ...
                bool ans = load_track_to_mixer_deck(track_title);\
    ...
    else
    {
        while (true)
        {
            std::string select = display_playlist_menu_from_config();
            if (select.empty())
            {
                break;
            }
            bool a = load_playlist(select);
            if (!a)
            {
                std::cout << "error";
                stats.errors++;
